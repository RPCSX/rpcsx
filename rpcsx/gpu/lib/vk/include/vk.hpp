...
struct Context {
  ...
  Context(Context &&other) noexcept { other.swap(*this); }
...
public:
  ...
  [[nodiscard]] VkDeviceMemory getHandle() const { return mDeviceMemory; }
...
public:
  ...
  Semaphore() = default;
  Semaphore(Semaphore &&other) noexcept { *this = std::move(other); }
  ...
  [[gnu::used, nodiscard]] std::uint64_t getCounterValue() const {
...
public:
  ...
  BinSemaphore(BinSemaphore &&other) noexcept { *this = std::move(other); }

  BinSemaphore &operator=(BinSemaphore &&other) noexcept {
    std::swap(mSemaphore, other.mSemaphore);
    return *this;
  }
  ...
  [[nodiscard]] VkSemaphore getHandle() const { return mSemaphore; }
...
public:
  ...
  Fence() = default;
  Fence(Fence &&other) noexcept { *this = std::move(other); }
...
public:
  ...
  CommandBuffer(CommandBuffer &&other) noexcept { *this = std::move(other); }
...
public:
  ...
  CommandBuffer createPrimaryBuffer(VkCommandBufferUsageFlags flags) {
    return {mHandle, VK_COMMAND_BUFFER_LEVEL_PRIMARY, flags};
  }

  operator VkCommandPool() const { return mHandle; }
  [[nodiscard]] VkCommandPool getHandle() const { return mHandle; }
...
public:
  ...
  [[nodiscard]] VkDeviceAddress getAddress() const { return mAddress; }
  [[nodiscard]] VkBuffer getHandle() const { return mBuffer; }
  ...
  [[nodiscard]] const DeviceMemoryRef &getMemory() const { return mMemory; }
...
public:
  ...
  [[nodiscard]] VkImageType getImageType() const { return mImageType; }
  [[nodiscard]] VkFormat getFormat() const { return mFormat; }
  [[nodiscard]] VkImageAspectFlags getAspects() const { return mAspects; }
  [[nodiscard]] std::uint32_t getWidth() const { return getExtent().width; }
  [[nodiscard]] std::uint32_t getHeight() const { return getExtent().height; }
  [[nodiscard]] std::uint32_t getDepth() const { return getExtent().depth; }
  [[nodiscard]] std::uint32_t getArrayLayers() const { return mArrayLayers; }
  [[nodiscard]] std::uint32_t getMipLevels() const { return mMipLevels; }
  [[nodiscard]] VkSampleCountFlagBits getSamples() const { return mSamples; }

  [[nodiscard]] VkImage getHandle() const { return mImage; }
...
public:
  ...
  ImageView() = default;
  ImageView(ImageView &&other) noexcept { *this = std::move(other); }
  ...
  [[nodiscard]] VkImageView getHandle() const { return mHandle; }
